...
namespace Framework.Web.Controls.MvcPager
{
    ...
    internal class PagerBuilder
    {
        ...
        private string GenerateUrl(int pageIndex)
        {
            ...
            if (pageIndex == 0)
                ...
                routeValues[_pagerOptions.PageIndexParameterName] = string.Empty;
            else
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            {
                return
                    MvcHtmlString.Create(string.Format("<div style=\"color:red;font-weight:bold\">{1} <a href='{0}'>【点击】返回第一页</a></div>",
                                         this.GenerateUrl(1), _pagerOptions.PageIndexOutOfRangeErrorMessage));
            }
            ...
            {
                sb.Append(BuildTotalItemCountPageSection(ref countScript));
            }
